...
- **Last-resort nft (SDD-004):** only if **both** links are gone and an operator passes `--enable` / `WAF_NFT_FALLBACK=1`. Recovery never installs nft. See [nft-dnat-fallback.md](nft-dnat-fallback.md).
...
| Both links unpinned | None (fail-closed) | Optional explicit nft DNAT (SDD-004); default OFF |
