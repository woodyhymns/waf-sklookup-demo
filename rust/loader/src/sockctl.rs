...
pub fn authorize(cred: PeerCred, owner: u32, group: u32, mode: u32) -> Result<()> {
    if mode & 0o007 != 0 { bail!("socket too open"); }
    if cred.uid == 0 || cred.uid == owner || cred.gid == group { return Ok(()); }
    bail!("unauthorized")
}
...
pub struct Request {
    ...
    #[serde(default)] pub count_only: bool,
...
pub fn start(path: PathBuf, group: Option<u32>, pin_dir: PathBuf, ports_file: PathBuf,
             shutdown: Arc<AtomicBool>, mutations: Arc<Mutex<()>>) -> Result<Server> {
    let parent = path.parent().filter(|p| !p.as_os_str().is_empty()).unwrap_or(Path::new("."));
    let parent_existed = parent.exists();
    fs::create_dir_all(parent).with_context(|| format!("create control socket directory {}", parent.display()))?;
    if !parent_existed {
...
fn peer_cred(stream: &UnixStream) -> Result<PeerCred> {
    let mut u = libc::ucred { pid: 0, uid: 0, gid: 0 };
    let mut len = std::mem::size_of::<libc::ucred>() as libc::socklen_t;
    let rc = unsafe { libc::getsockopt(stream.as_raw_fd(), libc::SOL_SOCKET, libc::SO_PEERCRED,
        (&mut u as *mut libc::ucred).cast(), &mut len) };
    if rc != 0 { return Err(std::io::Error::last_os_error()).context("SO_PEERCRED"); }
    Ok(PeerCred { pid: u.pid, uid: u.uid, gid: u.gid })
}

fn handle(mut stream: UnixStream, path: &Path, pin_dir: &Path, ports_file: &Path, mutations: &Mutex<()>) {
    let result = (|| -> Result<Value> {
        ...
        if line.len() > 1_048_576 { bail!("request too large"); }
...
fn execute(req: &Request, cred: PeerCred, pin_dir: &Path, ports_file: &Path, mutations: &Mutex<()>) -> Result<Value> {
    ...
    let actual_op = if op == "bulk" { req.action.as_deref().unwrap_or("").to_ascii_lowercase() } else { op.clone() };
    if actual_op == "fill" {
...
pub fn run_client(args: &[String]) -> Result<()> {
    ...
    let mut line=String::new(); BufReader::new(stream).read_line(&mut line)?; println!("{}", line.trim());
    let v: Value=serde_json::from_str(&line)?; if v.get("ok") != Some(&Value::Bool(true)) { bail!("control request failed"); } Ok(())
}
