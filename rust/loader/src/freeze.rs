...
pub fn is_frozen(path: &Path) -> bool { path.exists() }

pub fn set_frozen(path: &Path) -> Result<()> {
    let parent = path.parent().filter(|p| !p.as_os_str().is_empty()).unwrap_or_else(|| Path::new("."));
    fs::create_dir_all(parent).with_context(|| format!("create freeze directory {}", parent.display()))?;
...
pub fn reject_if_frozen(path: &Path, op: &str, tenant: &str, site: &str, ports: &[u16]) -> Result<()> {
    if is_frozen(path) {
...
mod tests {
    ...
    fn freeze_rejects_mutations_and_unfreeze_allows_again() {
        ...
        for op in ["add", "open", "bulk", "fill", "apply-central"] {
            let err = reject_if_frozen(&path, op, "demo", "local", &[18081]).unwrap_err().to_string();
